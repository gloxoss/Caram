...
export default function OrganizationSalesReturnReportPage() {
		...
		const fetchSalesReturnData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/sales-return?organizationId=${organizationId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Organization Sales Return Report" />
