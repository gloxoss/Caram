...
export default function OrganizationEmployeeSalesReportPage() {
		...
		const fetchEmployeeSalesData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/employee-sales?organizationId=${organizationId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Organization Employee Sales Report" />
