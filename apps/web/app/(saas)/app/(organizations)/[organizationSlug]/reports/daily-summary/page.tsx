...
export default function OrganizationDailySummaryReportPage() {
		...
		const fetchDailySummaryData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/daily-summary?organizationId=${organizationId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Organization Daily Summary Report" />
