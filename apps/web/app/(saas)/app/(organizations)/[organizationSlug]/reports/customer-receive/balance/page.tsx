...
export default function OrganizationCustomerReceiveBalanceReportPage() {
		...
		const fetchCustomerReceiveBalanceData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/customer-receive/balance?organizationId=${organizationId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Organization Customer Receive Balance Report" />
