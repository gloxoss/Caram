...
export default function OrganizationSupplierPaymentPage() {
		...
		const fetchSupplierPayments = async () => {
			...
			try {
				const response = await fetch(
					`/api/supplier-payments?organizationId=${organizationId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Organization Supplier Payments" />
