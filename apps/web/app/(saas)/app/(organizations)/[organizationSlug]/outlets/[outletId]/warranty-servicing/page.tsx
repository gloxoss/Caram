...
export default function OutletWarrantyServicingPage() {
		...
		const fetchWarrantyServicings = async () => {
			...
			try {
				const response = await fetch(
					`/api/warranty-servicings?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Warranty Servicings" />
