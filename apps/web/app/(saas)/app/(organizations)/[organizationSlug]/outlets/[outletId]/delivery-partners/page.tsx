...
export default function OutletDeliveryPartnersPage() {
		...
		const fetchDeliveryPartners = async () => {
			...
			try {
				const response = await fetch(
					`/api/delivery-partners?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Delivery Partners" />
