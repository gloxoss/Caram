...
export default function OutletExpenseCategoriesPage() {
		...
		const fetchExpenseCategories = async () => {
			...
			try {
				const response = await fetch(
					`/api/expense-categories?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Expense Categories" />
