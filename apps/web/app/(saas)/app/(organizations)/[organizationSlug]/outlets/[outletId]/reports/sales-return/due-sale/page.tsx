...
export default function OutletDueSaleReportPage() {
		...
		const fetchDueSaleData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/sales-return/due-sale?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Due Sale Report" />
