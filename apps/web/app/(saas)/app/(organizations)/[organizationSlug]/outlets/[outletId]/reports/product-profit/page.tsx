...
export default function OutletProductProfitReportPage() {
		...
		const fetchProductProfitData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/product-profit?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Product Profit Report" />
