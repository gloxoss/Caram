...
export default function OutletPurchaseReturnReportPage() {
		...
		const fetchPurchaseReturnData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/purchase-return?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Purchase Return Report" />
