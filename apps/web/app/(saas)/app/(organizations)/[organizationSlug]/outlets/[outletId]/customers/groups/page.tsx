...
export default function OutletCustomerGroupsPage() {
		...
		const fetchCustomerGroups = async () => {
			...
			try {
				const response = await fetch(
					`/api/customer-groups?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Customer Groups" />
