...
export default function OutletServiceSalesReportPage() {
		...
		const fetchServiceSalesData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/sales-return/service-sale?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Service Sales Report" />
