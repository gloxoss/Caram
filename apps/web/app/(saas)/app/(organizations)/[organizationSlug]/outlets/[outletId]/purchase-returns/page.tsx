...
export default function OutletPurchaseReturnsPage() {
		...
		const fetchPurchaseReturns = async () => {
			...
			try {
				const response = await fetch(
					`/api/purchase-returns?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Purchase Returns" />
