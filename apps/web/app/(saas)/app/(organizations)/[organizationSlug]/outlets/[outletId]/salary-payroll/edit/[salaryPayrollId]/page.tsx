...
export default function OutletEditSalaryPayrollPage() {
	...
	useEffect(() => {
		const fetchSalaryPayroll = async () => {
			...
			try {
				const response = await fetch(
					`/api/salary-payrolls/${salaryPayrollId}?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
		...
		fetchSalaryPayroll();
	}, [organizationId, outletId, salaryPayrollId]);
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Edit Outlet Salary Payroll" />
