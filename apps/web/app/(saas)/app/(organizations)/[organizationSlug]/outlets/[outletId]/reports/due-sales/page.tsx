...
export default function OutletDueSalesReportPage() {
		...
		const fetchDueSalesData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/due-sales?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Due Sales Report" />
