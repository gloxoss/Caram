...
export default function OutletCustomerReceiveLedgerReportPage() {
		...
		const fetchCustomerReceiveLedgerData = async () => {
			...
			try {
				const response = await fetch(
					`/api/reports/customer-receive/ledger?organizationId=${organizationId}&outletId=${outletId}`,
				);
				if (!response.ok) {
	...
	return (
		<div className="container mx-auto py-6">
			<PageHeader title="Outlet Customer Receive Ledger Report" />
