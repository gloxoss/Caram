		...
		async (c) => {
			...
			try {
				...
				if (
					...
					shipment.deliveryPartner.trackingUrlTemplate
				) {
					trackingUrl =
						shipment.deliveryPartner.trackingUrlTemplate.replace(
							"{trackingNumber}",
							shipment.trackingNumber,
						);
				}
		...
		async (c) => {
			...
			try {
				...
				if (data.estimatedDelivery) {
					shipmentData.estimatedDelivery = new Date(
			...
			try {
				...
				if (data.status && data.status !== existingShipment.status) {
					const currentHistory =
						existingShipment.trackingHistory || [];
